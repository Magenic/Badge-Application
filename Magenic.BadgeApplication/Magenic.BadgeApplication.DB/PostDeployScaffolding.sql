...
WHEN MATCHED THEN 
    UPDATE SET [BadgeName] = Source.[badge_name],
               ...
               [BadgeApprovedBy] = Source.[badge_approved_by],
               [BadgeApprovedDate] = Source.[badge_approved_date]
WHEN NOT MATCHED BY TARGET THEN 
    INSERT ([BadgeId], [BadgeName], [BadgeTagLine], [BadgeDescription], [BadgeTypeId], [BadgeCreated], [BadgeEffectiveStart], [BadgeEffectiveEnd], [BadgePriority], [MultipleAwardPossible], [DisplayOnce], [ManagementApprovalRequired], [ActivityPointsAmount], [BadgeAwardValueAmount], [BadgeApprovedBy], [BadgeApprovedDate])
    VALUES ([badge_id], [badge_name], [badge_tag_line], [badge_description], [badge_type_id], [badge_created], [badge_effective_start], [badge_effective_end], [badge_priority], [multiple_awards_possible], [display_once], [management_approval_required], [activity_points_amount], [badge_award_value_amount], [badge_approved_by], [badge_approved_date]);
